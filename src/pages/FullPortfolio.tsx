      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="cases-grid">
          {filteredCases.map((caseItem, index) => (
            <div 
              key={caseItem.id}
              className="case-card-item group relative transform transition-all duration-500"
              onClick={() => openCaseModal(caseItem)}
              style={{ animationDelay: `${index * 0.1}s` }}
            >
              {/* Glow effect */}
              <div className="card-glow absolute -inset-1 bg-gradient-to-r from-electric-blue via-blue-400 to-electric-blue rounded-3xl opacity-0 group-hover:opacity-15 blur-2xl transition-opacity duration-500"></div>
              
              {/* Main Card */}
              <div className="card-inner relative bg-gradient-to-br from-slate-800/95 to-navy-900/95 backdrop-blur-sm rounded-3xl overflow-hidden border border-electric-blue/20 group-hover:border-electric-blue/60 transition-all duration-500 shadow-xl group-hover:shadow-2xl group-hover:shadow-electric-blue/20">
                {/* Gradient Header with animated background */}
                <div className="relative h-32 mb-6 overflow-hidden">
                  {/* Animated gradient background */}
                  <div className={`absolute inset-0 ${caseItem.gradientClass} opacity-30 group-hover:opacity-50 transition-opacity duration-500`}></div>
                  
                  {/* Floating particles effect */}
                  <div className="absolute inset-0">
                    <div className="absolute top-4 left-4 w-2 h-2 bg-white/30 rounded-full animate-pulse"></div>
                    <div className="absolute top-8 right-12 w-1 h-1 bg-white/40 rounded-full animate-pulse" style={{ animationDelay: '0.5s' }}></div>
                    <div className="absolute bottom-6 left-20 w-1.5 h-1.5 bg-white/20 rounded-full animate-pulse" style={{ animationDelay: '1s' }}></div>
                  </div>
                  
                  {/* Status indicator with pulse */}
                  <div className="absolute top-4 right-4 z-10">
                    <div className={`w-4 h-4 rounded-full ${caseItem.status === 'completed' ? 'bg-emerald-400' : 'bg-amber-400'} shadow-lg relative`}>
                  ...
                  <div className="absolute bottom-4 left-4">
                    <span className={`px-4 py-1.5 rounded-full text-xs font-bold ${caseItem.categoryClass} backdrop-blur-sm border border-white/20 shadow-lg`}>
                      {caseItem.category}
                    </span>
                  </div>
                  
                  {/* Large Icon with bounce */}
                  <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-10">
                    <div className={`w-16 h-16 rounded-2xl ${caseItem.categoryClass.replace('text-', 'bg-gradient-to-br from-').replace('/20', '/30').replace('/80', '/40')} backdrop-blur-sm border border-white/20 flex items-center justify-center group-hover:scale-110 group-hover:rotate-3 transition-all duration-500 shadow-lg`}>
                ...
                <div className="p-6 pt-0">
                  {/* Title with gradient */}
                  <h3 className="text-xl font-bold mb-3 bg-gradient-to-r from-white to-slate-300 bg-clip-text text-transparent group-hover:from-electric-blue group-hover:to-blue-400 transition-all duration-300">
                    {caseItem.title}
                  </h3>
                  ...
                  {/* View button */}
                  <div className="flex items-center text-electric-blue text-sm font-semibold opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    <span>Подробнее</span>
      ...
      {selectedCase && (
        ...
        >
          <div 
            ...
            className="bg-gradient-to-br from-white to-slate-50 rounded-2xl md:rounded-3xl max-w-4xl w-full max-h-[90vh] overflow-y-auto shadow-2xl animate-slideUp border border-gray-200"
          >
            <div className="p-4 sm:p-6 lg:p-8">
              <div className="flex justify-between items-start mb-4 sm:mb-6">
                <h3 className="text-xl sm:text-2xl lg:text-3xl font-bold bg-gradient-to-r from-gray-900 to-gray-700 bg-clip-text text-transparent pr-2">
                  {selectedCase.title}
                </h3>
                <button 
                  onClick={closeCaseModal}
                  className="text-gray-400 hover:text-gray-600 text-xl sm:text-2xl hover:rotate-90 transition-transform duration-300 p-1 sm:p-2 flex-shrink-0"
                >
